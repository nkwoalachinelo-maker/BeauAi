            ...
            {analysis.products.map((p) => (
              <li key={`${p.product}-${p.shade}`}>
                <p className="font-medium">
                  <a href={`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(`${p.product} ${p.shade}`)}`} target="_blank" rel="noopener noreferrer" className="text-primary underline underline-offset-2">{p.product}</a>{" "}
